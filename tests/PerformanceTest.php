...
namespace web36\LaravelPerformance\Tests;

use web36\LaravelPerformance;
...
class PerformanceTest extends TestCase
{
    ...
    {
        $this->unit->timeStart($this->counterKey1);

        usleep(random_int(100, 100000));

        for ($i = 1; $i <= 5; $i++) {
            $this->unit->timeStart($this->counterKey2);
            usleep(random_int(100, 100000));
            $this->unit->timeEnd($this->counterKey2);
        }

        $this->unit->timeEnd($this->counterKey1);
